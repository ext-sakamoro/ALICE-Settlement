# ALICE-Settlement — Claude Code 設定

## プロジェクト概要

Post-trade settlement netting and clearing engine

| 項目 | 値 |
|------|-----|
| クレート名 | `alice-settlement` |
| バージョン | 0.1.0 |
| ライセンス | AGPL-3.0-only |
| リポジトリ | `ext-sakamoro/ALICE-Settlement` |
| Eco-Systemブリッジ | `bridge_settlement_ext.rs` |
| features | `std` (default), `ffi` (C FFI) |
...
- Path K (Financial→Settlement→DB)
